        ...
        if (player.getSession() != null && player.isAdmin()) {
            ...
            if (text.equals("arriety")) {
                NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_ADMIN, -1,
                        "Quản trị admin Arriety: " + Client.gI().getPlayers().size() + "\n",
                ...
                if (newPass.equals(rePass)) {
                    ...
                    try {
                        GirlkunDB.executeUpdate("update account set password = ? where id = ? and username = ?",
                                rePass, player.getSession().userId, player.getSession().uu);// Util.md5(rePass) fixed
                                                                                            // md5
